		...
		<div id="page-content" class="HBoard">
			<c:if test="${fn:length(list) gt 0}">
				<c:forEach var="item" begin="0" end="${fn:length(list) -1}" varStatus="st">
					<div class="inner-box HArticle ${(list.get(item).status == 99)?'sold':''}" onclick="location.href='<%=request.getContextPath() %>/board/details.do?bIdx=${list.get(item).bIdx}&aIdx=${list.get(item).aIdx}&page=${param.page}'">
			...
			<c:if test="${login != null}">
				<button class="normal-button accent-button" onclick="location.href='<%=request.getContextPath() %>/board/register.do?bIdx=4'">글쓰기</button>
			</c:if>
			</div>
